...
func Discover(graphOptions string, searchResults map[string][]string, kind string) error {
	...
	claimSvc := graphdb.NewClaimService(driver)

	if kind == "CLAIM" {
		for claim, apps := range searchResults {
			// For claim and apps create the nodes and claim relationships
			// Check if claim is already created?
			c, err := claimSvc.FindOneByName(claim)
			// Check for record not being found
			if domainError, ok := err.(*graphdb.DomainError); ok && domainError.StatusCode() == graphdb.NotFound {
				// Create Claim
				c, err = claimSvc.Save(claim)
			}
			...
			for _, app := range apps {
				...
				_, err = claimSvc.SaveRelation(graphdb.Access{Read: true}, a.AppId, c["claimId"].(string))
				if err != nil {
					//return err
				}
			}
